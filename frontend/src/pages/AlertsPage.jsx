...
function AlertsPage() {
  ...
  return (
    <div className={`space-y-6 ${darkMode === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>
      ...
      <div className={`dashboard-card  text-sm`}>
        <div className="flex items-center mb-4">
          <AlertCircle className={`h-5 w-5 ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-700'} mr-2`} />
          <h3 className={`text-lg font-medium ${darkMode === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>All Alerts</h3>
        </div>

        {loading ? (
          <div className={`text-gray-500 ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Loading alerts...</div>
        ) : alerts.length === 0 ? (
          <div className={`text-gray-500 ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>No alerts found.</div>
        ) : (
          <ul className="space-y-2">
            {alerts.map((alert) => (
              <li
                key={alert.id || Math.random()} // fallback key if id is missing
                className={`p-3 rounded-md ${darkMode === 'dark' ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-700'}`}
              >
                <div
                  className={`font-semibold inline-block px-2 py-1 rounded-md text-sm ${
                    alert.severity === 'critical'
                ...
                <div>{alert.message || 'No message provided.'}</div>
                <div className={`text-xs ${darkMode === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                  {alert.timestamp
                    ? new Date(alert.timestamp).toLocaleString()
                    : 'Unknown time'}
                </div>
