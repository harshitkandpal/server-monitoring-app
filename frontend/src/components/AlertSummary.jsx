...
import { AlertCircle } from 'lucide-react';
import axios from 'axios';
import { useTheme } from '../components/ThemeContext'; // Import the custom theme hook

function AlertSummary() {
  const { darkMode } = useTheme(); // Get the current darkMode state
  const [alertData, setAlertData] = useState({
    critical: 0,
    medium: 0,
    low: 0,
  ...
  if (alertData.loading) {
    return (
      <div className={`dashboard-card flex items-center justify-center h-32 ${darkMode === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-500'}`}>
  ...
  if (alertData.error) {
    return (
      <div className={`dashboard-card flex items-center justify-center h-32 ${darkMode === 'dark' ? 'bg-gray-800 text-red-500' : 'bg-white text-red-500'}`}>
        <div>{alertData.error}</div>
      </div>
    );
  }
  ...
  return (
    <div className={`dashboard-card `}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className={` p-4 rounded-lg overflow-hidden ${darkMode === 'dark' ? 'bg-red-500 text-white' : 'bg-red-100 text-black'}`}>
          <div className="flex items-center justify-between">
        ...
        <div className={` p-4 rounded-lg overflow-hidden ${darkMode === 'dark' ? 'bg-yellow-500 text-white' : 'bg-yellow-100 text-black'}`}>
          <div className="flex items-center justify-between">
        ...
        <div className={` p-4 rounded-lg overflow-hidden ${darkMode === 'dark' ? 'bg-green-500 text-white' : 'bg-green-100 text-black'}`}>
          <div className="flex items-center justify-between">
