...
function ServerTable() {
  ...
  return (
    <div className={`dashboard-card overflow-hidden ${darkMode === 'dark' ? ' text-gray-100' : ' text-gray-800'}`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium">Server Status</h3>
        <Link to="/servers" className="text-sm text-blue-600 hover:text-blue-800">
          View all
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className={`bg-gray-50 ${darkMode === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
            <tr>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Server
              </th>
          ...
          <tbody className={` divide-y divide-gray-200 ${darkMode === 'dark' ? 'bg-gray-700' : 'bg-white'}`}>
            {servers.data.map(server => (
              <tr key={server.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <Link to={`/servers/${server.id}`} className="text-blue-600 hover:text-blue-900">
