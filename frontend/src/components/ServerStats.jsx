...
function ServerStats() {
  ...
  const getChartData = (value, label, color) => {
    return {
      ...
      datasets: [
        {
          data: [value, 100 - value],
          backgroundColor: [color, '#e5e7eb'], // Lighter background for the available portion
          borderWidth: 0,
          cutout: '70%'
        },
      ],
  ...
  return (
    <div className={`dashboard-card ${darkMode === 'dark' ? ' text-gray-100' : ' text-gray-800'}`}>
      <div className="flex items-center mb-4">
        <Activity className={`h-5 w-5 ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-700'} mr-2`} />
        <h3 className="text-lg font-medium">Server Usage</h3>
      </div>
      
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {/* CPU Usage */}
        <div className={`p-4 rounded-lg flex flex-col items-center ${darkMode === 'dark' ? 'bg-blue-900' : 'bg-blue-50'}`}>
          <div className="h-24 w-24 relative mb-2">
            <Doughnut 
              data={getChartData(statsData.cpu_usage, 'CPU', '#3b82f6')} 
              options={chartOptions}
          ...
          <span className={`text-sm font-medium ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>CPU</span>
        </div>
        
        {/* RAM Usage */}
        <div className={`p-4 rounded-lg flex flex-col items-center ${darkMode === 'dark' ? 'bg-purple-900' : 'bg-purple-50'}`}>
          <div className="h-24 w-24 relative mb-2">
            <Doughnut 
              data={getChartData(statsData.ram_usage, 'RAM', '#8b5cf6')} 
              options={chartOptions}
          ...
          <span className={`text-sm font-medium ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>RAM</span>
        </div>
        
        {/* Disk Usage */}
        <div className={`p-4 rounded-lg flex flex-col items-center ${darkMode === 'dark' ? 'bg-green-900' : 'bg-green-100'}`}>
          <div className="h-24 w-24 relative mb-2">
            <Doughnut 
              data={getChartData(statsData.disk_usage, 'Disk', '#10b981')} 
              options={chartOptions}
          ...
          <span className={`text-sm font-medium ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Disk</span>
        </div>
      </div>
      
      <div className="mt-4">
        <h4 className={`text-sm font-medium ${darkMode === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-2`}>Application Performance</h4>
        <div className="h-4 bg-gray-200 rounded-full">
          <div 
            className="h-4 bg-blue-600 rounded-full" 
            style={{ 
